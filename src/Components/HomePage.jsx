...
function HomePage() {
  ...
  const handlePreviousArticle = (topicSlug) => {
    setCurrentArticleIndex((prevIndexes) => ({
      ...prevIndexes,
      [topicSlug]:
        (prevIndexes[topicSlug] - 1 + articles[topicSlug].length) %
        articles[topicSlug].length,
    }));
  };
                  ...
                  <div key={topic.slug}>
                    <div className="topic-tile flex lg:flex-row flex-col">
                      ...
                      <div className="side-titles-container">
                        {[1, 2].map((offset) => {
                          const index =
                            (currentArticleIndex[topic.slug] + offset) %
                            articles[topic.slug].length;
                          return (
                            <Link
                            ...
                            >
                              {capitalizeTitle(
                                articles[topic.slug][index].title
                              )}
                            </Link>
                          );
                        })}
